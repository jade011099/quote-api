...
def debug_gemini():
    if not GEMINI_API_KEY:
        return jsonify(ok=False, error="GEMINI_API_KEY 未設定。請在 Render Environment Variables 加入 GEMINI_API_KEY。"), 500
    ...
    try:
        r = requests.post(url, json=payload,
                          headers={"Content-Type": "application/json",
                                   "x-goog-api-key": GEMINI_API_KEY}, timeout=30)
        if r.ok:
            return jsonify(ok=True, model=GEMINI_MODEL, message="Gemini API key works ✅")

        body = r.json() if r.headers.get("content-type", "").startswith("application/json") else {}
        detail = body.get("error", {}).get("message", r.text[:300])

        if r.status_code == 401:
            msg = "Gemini API key 無效或已過期。請到 Google AI Studio 確認 API key，並更新 Render 的 GEMINI_API_KEY。"
        elif r.status_code == 403:
            msg = f"Gemini API 權限不足或 key 被限制。Detail: {detail}"
        elif r.status_code == 404:
            msg = f"找不到模型 {GEMINI_MODEL}。請在 Render 將 GEMINI_MODEL 改為可用的模型（例如 gemini-2.0-flash）。"
        elif r.status_code == 429:
            msg = f"Gemini API 配額超限。請稍後再試或升級 billing。Detail: {detail}"
        else:
            msg = f"Gemini API 回傳 {r.status_code}: {detail}"

        return jsonify(ok=False, status_code=r.status_code, error=msg, gemini_key_prefix=GEMINI_API_KEY[:6] + "..."), r.status_code

    except requests.exceptions.Timeout:
        return jsonify(ok=False, error="Gemini API 連線逾時"), 504
...
def ocr():
    ...
    if not GEMINI_API_KEY:
        return jsonify(error="伺服器未設定 GEMINI_API_KEY。請在 Render Environment Variables 加入 GEMINI_API_KEY。"), 500
    ...
    try:
        ...
        if not r.ok:
            ...
            if r.status_code == 401:
                return jsonify(error="Gemini API key 無效或已過期。請到 Google AI Studio 確認 API key，並更新 Render 的 GEMINI_API_KEY。", status_code=401), 401
            elif r.status_code == 404:
                return jsonify(error=f"找不到模型 {GEMINI_MODEL}。請在 Render 將 GEMINI_MODEL 改為可用的模型。", status_code=404), 404
            elif r.status_code == 429:
                return jsonify(error=f"Gemini API 配額超限，請稍後再試。{detail}", status_code=429), 429
            else:
                return jsonify(error=f"Gemini API 錯誤 {r.status_code}: {detail}", status_code=r.status_code), r.status_code
...
def export_xlsx():
    ...
    if not os.path.exists(TEMPLATE):
        return jsonify(error=f"找不到 Excel 模板檔：{TEMPLATE}"), 500
    try:
        buf = _fill_xlsx(data)
        ...
        return jsonify(error=f"Excel 匯出失敗：{str(e)}"), 500
...
def export_pdf():
    ...
    try:
        ...
        return jsonify(error=f"PDF 匯出失敗：{str(e)}"), 500
